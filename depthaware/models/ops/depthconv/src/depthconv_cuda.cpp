...
//                     columns.data(), n);
    ...
    torch::Tensor product = torch::matmul(gradOutput.view({gradOutput.size(0),gradOutput.size(1), gradOutput.size(2)*gradOutput.size(3)}).transpose(2, 1), gradWeight_flat);
